...
* **צ'אט קהילתי בלייב:** השתתפות פעילה בצ'אט הקהילתי בזמן אמת (Socket.IO) לצפייה בהודעות ושידור תגובות.

#### 2. משתמש VIP (VIP Premium Collector):
* **הרשאות מורחבות:** כל פונקציונליות המשתמש הסטנדרטי, בנוסף לאחסון פריטים ללא הגבלה בכספת.
* **מערכת מסחר והחלפות בלעדית (Trade Engine & Trade Manager):** יצירת הצעות סחר מורכבות מתוך כספות ציבוריות, ניהול הצעות נכנסות ויוצאות במרכז ההחלפות (`/TradeManager`), וביצוע החלפת בעלות אטומית ומודעת-כמויות ב-DB.
* **חיפוש אספנים מתקדם ושיחות פרטיות:** גישה למנוע חיפוש אספנים המאפשר צפייה באוספים ציבוריים של משתמשים אחרים במערכת (`PublicVault.jsx`) ופתיחת שיחות פרטיות 1-on-1.
* **תגי יוקרה והתראות:** הצגת תג VIP יוקרתי (VIP Badge 👑) בכל אזורי הקהילה והצ'אט, וקבלת התראות יוקרה בלעדיות ("🔥 Live Grail Alerts").

#### 3. מנהל מערכת (Admin / Business Owner):
* **דשבורד מנהלים עסקי (Business Dashboard):** צפייה בסטטיסטיקות מערכת כוללות: כמות משתמשים רשומים, פילוח משתמשי חינם מול VIP, הכנסות משודרגות מ-Stripe, וכמות פריטים כוללת במערכת.
* **ניהול משתמשים ותפקידים (User Management & Role Toggle):** צפייה ברשימת המשתמשים הרשומים במערכת ועדכון תפקידם בלייב בין משתמש רגיל למשתמש VIP בלחיצת כפתור דרך ממשק ה-Admin Panel. (הרשאות מנהל-על, חסימת משתמשים ואיפוס סיסמאות מנוהלים ישירות ברמת מסד הנתונים).
...
4. **פערי תצורה ומשתני סביבה בין סביבת הפיתוח לייצור (Dev vs. Prod Environment Configuration):**
   * *הבעיה:* לאחר העלאת המערכת לשרת הענן (AWS EC2), קישורים המיוצרים דינמית על ידי השרת (כגון קישורי אימות דוא"ל מ-Brevo) הופנו לכתובת ה-IP של השרת ולפורטים של סביבת הפיתוח (למשל `http://54.145.50.157:8080`) במקום לדומיינים המאובטחים. הדבר הוביל לשגיאת `ERR_CONNECTION_REFUSED` בצד הלקוח, מאחר ששרת ה-Nginx מאזין ומנתב בקשות בפורטים 80/443 בלבד.
...
4. **פתרון פערי התצורה:** הטמעת מנגנון Fallback קשיח בקוד השרת המפנה תמיד לדומיינים של הייצור (`https://api.mypopvault.online` ו-`https://mypopvault.online`), במקביל להזרקת משתני סביבה מוחלטים (`API_URL`, `FRONTEND_URL`) ישירות לקובץ ה-`.env` המוגן בשרת ה-AWS. פעולה זו ניתקה את תלות השרת בכתובות IP מקומיות והבטיחה יצירת קישורים מאובטחים, תוך שמירה על בידוד משתני הסביבה ממאגר ה-GitHub (באמצעות `.gitignore`).
