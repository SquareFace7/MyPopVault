...
עבור כל מסך במערכת מוגדרים: תפקיד המשתמש, מטרת המסך (הבעיה שהוא פותר), והפעולות המרכזיות המבוצעות בו:

### מסך 1: דף נחיתה (Landing Page)
* **מי המשתמש במסך:** אורח / משתמש לא רשום.
* **מטרת המסך:** הצגת ערך המערכת, מודל ה-Freemium, והנעת המשתמש להרשמה.
* **מה המשתמש עושה בפועל במסך:** צופה בסרטון הסבר ובתכונות מרכזיות, צופה בפריטים פופולריים, ולוחץ על כפתור הרשמה/התחברות.

### מסך 2: מסך התחברות (Login Screen)
* **מי המשתמש במסך:** משתמש רשום קיים.
* **מטרת המסך:** אימות זהות המשתמש והנפקת אסימון JWT מאובטח.
* **מה המשתמש עושה בפועל במסך:** מזין כתובת דוא"ל וסיסמה, בוחר באפשרות "זכור אותי", ולוחץ על התחברות.

### מסך 3: מסך הרשמה (Signup Screen)
* **מי המשתמש במסך:** משתמש חדש.
* **מטרת המסך:** יצירת חשבון חדש במערכת והגדרת פרופיל אספן בסיסי.
* **מה המשתמש עושה בפועל במסך:** מזין שם משתמש, דוא"ל, סיסמה ואישור סיסמה, ומאשר את תנאי השימוש.

### מסך 4: מסך איפוס סיסמה (Forgot / Reset Password)
* **מי המשתמש במסך:** משתמש ששכח את סיסמתו.
* **מטרת המסך:** שחזור גישה מאובטח לחשבון ללא התערבות מנהל.
* **מה המשתמש עושה בפועל במסך:** מזין כתובת דוא"ל לקבלת קישור חד-פעמי, ולאחר מכן מזין סיסמה חדשה בדף האיפוס.

### מסך 5: דשבורד ניהול אוסף אישי ואנליטיקות (Collector Dashboard)
* **מי המשתמש במסך:** אספן רשום (Standard / VIP).
* **מטרת המסך:** ריכוז תמונה פיננסית וויזואלית מלאה של האוסף האישי בזמן אמת.
* **מה המשתמש עושה בפועל במסך:** צופה במדדי שווי כולל, סך השקעה ורווח/הפסד (ROI), צופה בגרף התפלגות קטגוריות, ומזהה את הפריט היקר ביותר באוסף שלו.

### מסך 6: מסך ניהול כספת אישית (Personal Vault Collection)
* **מי המשתמש במסך:** אספן רשום.
* **מטרת המסך:** ניהול מלאי אישי (CRUD) על פריטי ה-Pop שנרכשו.
* **מה המשתמש עושה בפועל במסך:** מעדכן מחיר קנייה מקורי, מעדכן מצב קופסה (Mint/Damaged), מעדכן כמות, ומוחק פריטים מהכספת.

### מסך 7: מסך סייר הקטלוג (Pop Explorer)
* **מי המשתמש במסך:** כל המשתמשים.
* **מטרת המסך:** חיפוש, סינון וגילוי דמויות Pop מתוך הקטלוג הגלובלי המעודכן.
* **מה המשתמש עושה בפועל במסך:** מזין מילת חיפוש, מסנן לפי קטגוריה (Marvel, DC, Anime), צופה בדרגת נדירות ומחיר שוק, ולוחץ על "Add to Vault".

### מסך 8: מסך פרטי פריט קטלוגי (Pop Details Page)
* **מי המשתמש במסך:** כל המשתמשים.
* **מטרת המסך:** הצגת נתונים היסטוריים ומפרט טכני מלא עבור פריט Pop ספציפי.
* **מה המשתמש עושה בפועל במסך:** צופה בהיסטוריית שינויי מחיר, במספר הסדרה, בתמונות מוגדלות ובדרגת הנדירות.

### מסך 9: מסך חיפוש אספנים ואוספים ציבוריים (Collector Search & Public Vault)
* **מי המשתמש במסך:** משתמשי VIP בלבד.
* **מטרת המסך:** חשיפה קהילתית ואיתור אספנים אחרים למטרות יצירת קשר ומסחר.
* **מה המשתמש עושה בפועל במסך:** מחפש אספנים לפי שם, צופה בכספת הציבורית שלהם, ויוזם הצעת סחר או שיחת צ'אט.

### מסך 10: מסך מרכז ההחלפות והצעות סחר (Trade Manager)
* **מי המשתמש במסך:** אספנים רשומים.
* **מטרת המסך:** ניהול תהליכי החלפת פריטים (Barter Trading) בצורה מאובטחת.
* **מה המשתמש עושה בפועל במסך:** בוחר פריטים מהכספת שלו מול פריטים מבוקשים מאספן אחר, שולח הצעה, ומאשר/דוחה הצעות נכנסות.

### מסך 11: מסך צ'אט קהילתי ומסרים פרטיים (Community Chat & Direct Messenger)
* **מי המשתמש במסך:** אספנים רשומים.
* **מטרת המסך:** תקשורת ישירה בזמן אמת לתיאום עסקאות סחר ודיוני אספנות.
* **מה המשתמש עושה בפועל במסך:** שולח הודעות טקסט פרטיות, צופה בסטטוס קריאה, ומתאם מפגשי החלפה.

### מסך 12: מסך שדרוג מנוי VIP (Vip Upgrade Page)
* **מי המשתמש במסך:** אספן סטנדרטי המעוניין לשדרג את חשבונו.
* **מטרת המסך:** הצגת יתרונות מנוי ה-VIP והנעה לתשלום מול Stripe.
* **מה המשתמש עושה בפועל במסך:** צופה ברשימת ההטבות (אחסון בלתי מוגבל, תג VIP), ולוחץ על כפתור מעבר לסליקה.

### מסך 13: מסכי אישור/ביטול תשלום (VIP Success & Cancel Pages)
* **מי המשתמש במסך:** משתמש שביצע/ביטל תהליך תשלום ב-Stripe.
* **מטרת המסך:** מתן משוב ברור על סטטוס התשלום והדרכת המשתמש על הצעדים הבאים.
* **מה המשתמש עושה בפועל במסך:** צופה הודעת אישור על שדרוג החשבון ל-VIP או הודעה על ביטול העסקה וחזרה לדף הבית.

### מסך 14: מסך ממשק מנהל מערכת (Admin Control Panel)
* **מי המשתמש במסך:** מנהלי מערכת (Admins).
* **מטרת המסך:** שליטה מלאה בנתוני האתר, ניהול משתמשים וקטלוג המערכת.
* **מה המשתמש עושה בפועל במסך:** צופה בסטטיסטיקות אתר כוללות, משנה תפקידי משתמשים, חוסם משתמשים מפרים, ומעדכן פריטים בקטלוג המרכזי.
